...
func getIndex(w http.ResponseWriter, r *http.Request) {
	...
	err := db.Select(&results, "SELECT p.id AS `id`, p.user_id AS `user_id`, p.body AS `body`, p.mime AS `mime`, p.created_at AS `created_at`, u.account_name AS `user.account_name` FROM `posts` p JOIN `users` u ON p.user_id=u.id WHERE u.del_flg=0 ORDER BY `created_at` DESC LIMIT 20")
...
func getPosts(w http.ResponseWriter, r *http.Request) {
	...
	err = db.Select(&results, "SELECT p.id AS `id`, p.user_id AS `user_id`, p.body AS `body`, p.mime AS `mime`, p.created_at AS `created_at`, u.account_name AS `user.account_name` FROM `posts` p JOIN `users` u ON p.user_id=u.id WHERE p.created_at <= ? AND u.del_flg=0 ORDER BY `created_at` DESC LIMIT 20", t.Format(ISO8601Format))
	if err != nil {
		log.Print(err)
		return
	}
